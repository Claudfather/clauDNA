...
| Skill | Description |
|-------|-------------|
| `/audit` | Codebase-audit engine — `security` / `tech-debt` / `docs` / `design` / `access-path` / `data-model` / `data-model-redesign` / `frontend-perf` / `repo-health` / `system` / `cache` / `multi-tenancy` lenses (replaces the eight standalone audit skills) |
