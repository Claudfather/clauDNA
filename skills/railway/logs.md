...
Invoked by /claudna:railway in logs mode. Pre-flight (CLI install, version gate, auth, project link) has already passed per the engine contract — do not re-run it. Any remaining dispatch arguments are the service name or filter to apply.

View and filter Railway service logs. Supports JSON output, level filtering, time ranges, and pattern matching. Follow these steps in order.
