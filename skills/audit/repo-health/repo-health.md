...
- For `docs`: author the dashboard and priority recommendations as a publishable doc in scratch, then `/claudna:publish <file> --to docs --dir documentation/planning/repo_health/<session_name>_<YYYY-MM-DD>/` (underscore form, matching the `tech_debt/` planning-dir convention)
