...
This is the binding contract for every agent in clauDNA. Adding or modifying an agent means satisfying these rules. Pull requests that violate the contract are rejected by CI ([`scripts/validate-agents.py`](./scripts/validate-agents.py), part of the `make check` check-set CI runs).
