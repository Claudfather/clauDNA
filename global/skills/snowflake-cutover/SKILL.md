...
description: "Use when you need to migrate a project's Snowflake connection to use internal-python-tools with RSA key-pair auth support."
...
# Snowflake Connection Cutover to internal-python-tools

Migrate a project from custom Snowflake connection logic to the centralized
`internal-python-tools` credential adapter pattern with RSA key-pair auth support.
...
2. **Check internal-python-tools dependency** — Install with `--no-deps` if missing; add to build command, not pyproject.toml deps
...
- internal-python-tools pins `fastapi==0.115.12` — always install with `--no-deps` to avoid conflicts
