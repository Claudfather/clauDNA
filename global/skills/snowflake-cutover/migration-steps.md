...
## Step 2: Check if internal-python-tools is already a dependency

```
grep -r "internal.python.tools\|internal_python_tools" pyproject.toml setup.py requirements*.txt
```

If not present, install with `--no-deps` to avoid transitive dependency conflicts:
```
pip install --no-deps "internal-python-tools @ git+https://github.com/example-org/internal-python-tools.git"
```

Add to build command (Railway, Dockerfile, etc.) — NOT to pyproject.toml deps
(internal-python-tools pins fastapi==0.115.12 which conflicts with most projects).
...
```python
"""Snowflake credentials adapter using internal-python-tools."""
from __future__ import annotations

import logging

from internal_python_tools.snowflake_query_plan_visualizer.connection import (
...
- Remove local credential validation logic (e.g., "if no password and no key, raise error")
  — internal-python-tools handles this at connection time
- If the project has a SecretsManager or similar that validated Snowflake credentials
  separately, simplify it to delegate to internal-python-tools
...
- Remove tests that validated specific auth methods (now handled by internal-python-tools)
