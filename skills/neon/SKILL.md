...
1. **CLI installed** — `query` needs `psql --version` only (neon is not required). `branch` and `info` need neon: probe `neon --version`, fallback `npx neon --version` (separate parallel Bash calls); if only the fallback works, prefix every neon command with `npx`. Both also use `psql`/`pg_isready` for connection tests.
