...
3. **Engines carry verbs.** Consolidated capabilities live as modes, not skills: infra operations are `/claudna:modal|railway|vercel|neon <verb>`, audits are `/claudna:audit <lens>`, session continuity is `/claudna:session <mode>`, review is `/claudna:review-work <mode>`, and the review panel's lenses run via `/claudna:ironclad --lens <name>`. If a remembered skill name is missing, its successor's description carries a `Replaces /old-name` breadcrumb — search descriptions for the old name.
